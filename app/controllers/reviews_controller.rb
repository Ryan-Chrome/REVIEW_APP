class ReviewsController < ApplicationController
    ...
    def index
        if !params[:search].present? && !params[:category].present?
            ...
            @reviews_category.each do |category|
                review_count = Review.where("category = ?", category[0]).count
                category[1] = review_count
            end
        end
        if params[:search].present?
            @reviews = Review.where("name LIKE ?", "%#{params[:search]}%").paginate(page: params[:page], per_page: 40)
        end
        if params[:category].present?
            @reviews = Review.where("category = ?", params[:category]).paginate(page: params[:page], per_page: 40)
        end
    end
    ...
    private
        ...
        def params_review
            params.require(:review).permit(:name, :category, :rate, :content, :image1, :image2, :image3, :image4, :image5)
        end
